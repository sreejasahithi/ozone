...
public final class StorageContainerLocationProtocolClientSideTranslatorPB
  ...
  /**
   * Creates a new StorageContainerLocationProtocolClientSideTranslatorPB with a NodeIdHolder.
   *
   * @param proxyProvider {@link SCMContainerLocationFailoverProxyProvider}
   * @param holder {@link NodeIdHolder} to route requests to specific SCM nodes
   */
  public StorageContainerLocationProtocolClientSideTranslatorPB(
      SCMContainerLocationFailoverProxyProvider proxyProvider, NodeIdHolder holder) {
    Objects.requireNonNull(proxyProvider, "proxyProvider == null");
    this.fpp = proxyProvider;
    this.nodeIdHolder = holder;
  ...
  private ScmContainerLocationResponse submitRpcRequest(
    ...
    // If nodeIdHolder has a specific node ID, route request to that node
    if (nodeIdHolder != null && nodeIdHolder.hasNodeId()) {
      try {
        StorageContainerLocationProtocolPB proxy = fpp.getProxyForNode(nodeIdHolder.getNodeId());
        return proxy.submitRequest(NULL_RPC_CONTROLLER, wrapper);
      } catch (IOException e) {
        throw new ServiceException("Failed to get proxy for node: " + nodeIdHolder.getNodeId(), e);
      }
    }
   ...
   */
  public static class NodeIdHolder {
